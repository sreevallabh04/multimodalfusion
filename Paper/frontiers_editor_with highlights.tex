...
\usepackage{fix-cm}
\usepackage{url,hyperref,lineno,caption}
...
\graphicspath{{./figures/}{../figures/}{../../figures/}{../statistial_testing/}{./statistial_testing/}}
...
\def\Address{$^{1}$School of Computer Science and Engineering, Vellore Institute of Technology, Chennai, India\\
...
The significant and persistent threat to the global food security is the crop diseases \cite{fao2021,fao2023,savary2019}, with annual losses approximately 10--16\% of global harvest value. Mango (\textit{Mangifera indica}), is the fifth most produced fruit crop worldwide and it is particularly vulnerable to the diseases such as anthracnose, alternaria, black mould rot, and stem-end rot. These diseases can rapidly spread and destroy the yields  through orchards if the detection not done in early periods \cite{singh2019}. For ffective crop management and economic sustainability, timely detection and accurate disease diagnosis is essential.

In the recent years, deep learning has surfaced as a powerful tool for automated plant disease detection \cite{mohanty2016,ferentinos2018,thakur2022} from the images. Convolutional neural networks (CNNs) trained on RGB photographs can classify diseases in real time on smartphones, offering a low-cost alternative to laboratory analysis \cite{kamilaris2018,coulibaly2022}. However, RGB-only approaches typically achieve accuracies between 75--85\% and struggle with diseases that manifest primarily through internal tissue changes not visible on the surface \cite{pineda2021,abade2021}. Recent comprehensive reviews have highlighted that while deep learning models continue to improve in accuracy, the reliance on a single imaging modality remains a fundamental bottleneck \cite{thakur2022,hasan2024}.

\textcolor{blue}{Multi-modal imaging has shown remarkable improvement for improving detection accuracy \cite{khanal2017,maimaitijiang2020} especially the combination of RGB with thermal sensing.} Thermal cameras detect temperature variations caused by metabolic stress in diseased tissue, providing complementary information that is invisible to standard cameras \cite{mahlein2016,pineda2021}. Infected regions typically exhibit temperature elevations of 2--5$^{\circ}$C above healthy tissue baselines due to cell lysis and altered transpiration \cite{mahlein2016}. Despite these advantages, thermal imaging equipment costs typically exceed \$25,000, placing multi-modal solutions beyond the reach of smallholder farmers who produce the majority of the world's mangoes \cite{sishodia2020}.
...
\begin{itemize}
    ...
    \item A 16-head multi-head attention fusion module with per-modality learned gating, which selectively re-weights RGB and synthetic thermal features before and during cross-modal integration.
    
    \item Five-class mango disease classification experiments yielding 87.3\% accuracy---4.8 percentage points above the strongest RGB-only baseline (ViT-Base, 85.8\%) and roughly 89\% of what a dedicated LWIR thermal camera achieves, with no additional sensors required.
\end{itemize}
...
Over the past decade, the combination of RGB with supplementary sensing modalities has steadily improved plant disease detection reliability \cite{maimaitijiang2020,lu2020}. Among these modalities, thermal imaging is particularly valuable because fungal and bacterial infections elevate tissue temperature by 2--5$^{\circ}$C through cell lysis and disrupted transpiration \cite{mahlein2016}---signatures that often appear before any visible lesion forms \cite{pineda2021}. Hyperspectral and multispectral platforms extend this further; Nagasubramanian et al.\ \cite{nagasubramanian2019} demonstrated disease detection in soybeans using spectral reflectance alone. The practical obstacle is cost: calibrated LWIR cameras exceed \$25{,}000 and multispectral rigs can surpass \$50{,}000, making them inaccessible for the smallholder operations where crop losses are most acute \cite{sishodia2020}.

Deep learning has fundamentally changed what is achievable with RGB imagery alone \cite{kamilaris2018,coulibaly2022}. Convolutional backbones and vision transformers extract rich hierarchical features from standard photographs and, when trained at scale, push classification accuracy well above earlier hand-crafted approaches \cite{dosovitskiy2021,liu2022convnext}. Thakur et al.\ \cite{thakur2022} review this progress in detail, noting that the performance gap between RGB-only and multi-modal systems persists precisely because thermal and spectral bands capture subsurface stress that RGB cannot. For mango stem-end rot in particular, internal decay is well advanced before any surface discolouration appears, making unimodal RGB methods especially unreliable \cite{lu2020,pineda2021}.

\subsection{Cross-Modal Learning and Knowledge Transfer}
Bridging different sensing modalities or biological domains to share learned representations is a long-standing problem in computer vision \cite{zhuang2020}. Agricultural transfer learning has mostly stayed within a single modality or anatomy---fine-tuning ImageNet weights for leaf classification, for instance \cite{chen2020_transfer,tan2018,barbedo2019}. Hinton et al.'s knowledge distillation framework \cite{hinton2015} opened a route to transferring what one model has learned into another, and subsequent work has extended this idea to cross-modal scenarios where the source and target modalities are structurally different \cite{li2023_kd}.

Xu et al.\ \cite{xu2023} show through an extensive analysis that transformer attention is particularly effective at aligning representations across heterogeneous inputs, which is relevant here because the RGB and synthetic thermal streams need to be jointly weighted rather than naively concatenated. Physics-informed pipelines \cite{karniadakis2021} and attention-based agricultural fusion \cite{niu2021,guo2022} are complementary threads; Wang et al.\ \cite{wang2022_synthetic} and Ji et al.\ \cite{ji2023} illustrate that domain knowledge injected into the training objective improves disease recognition beyond purely data-driven baselines. Foundation models and vision transformers have also shown that features learned on one anatomy can transfer to another when the underlying pathology is consistent \cite{vaswani2017,dosovitskiy2021}---a property this work exploits from leaf to fruit. Recent studies \cite{xiao2025thermalgen,tulu2026rgb} further motivate RGB--thermal fusion for alignment and retrieval tasks where synchronized paired data is scarce.
...
\begin{figure}[!ht]
    ...
    \begin{minipage}{0.25\textwidth}
        ...
        \par{(d) Leaf with Anthracnose}
    \end{minipage}
    \captionof{figure}{Exemplar images showing that the diseases look similar on different plant organs for the MangoFruitDDS dataset (top row) and MangoLeafBD (bottom row) dataset  }
...
With the lesion detector trained on leaf data, we pass each MangoFruitDDS fruit image through it to obtain a spatial lesion probability map. Because diseased plant tissue runs metabolically hotter---cell lysis disrupts transpiration and elevates surface temperature by 2--5$^{\circ}$C \cite{mahlein2016,pineda2021}---these probabilities can be mapped to plausible pseudo-thermal values through a simple physically motivated model \cite{raissi2019,karniadakis2021}. The synthesis equation is:
\endgroup
...
\textbf{Training duration with early stopping:} Models were trained for up to 50 epochs with validation accuracy monitored throughout. Training was stopped when accuracy showed no improvement for 15 consecutive epochs; in practice, convergence occurred between epochs 30 and 40 (see Figure~\ref{fig:accuracy}). This ceiling avoids both the under-training that results from too few epochs and the overfitting that accumulates with unnecessary additional passes over a small dataset.

\textbf{Regularization:} Weight decay of $10^{-4}$ is applied to all parameter groups. The fusion model's cross-entropy loss uses label smoothing of 0.1, and gradient norms are clipped at 1.0 to prevent instability in the early stages of training.

\textbf{Fusion training protocol:} RGB branch weights, pretrained in isolation, are loaded into the dual-branch model before fusion training begins. For the first 15 epochs the RGB branch is frozen, so only the thermal branch and the fusion layers receive gradient updates. The entire network is then unfrozen and fine-tuned end-to-end at one-tenth of the original learning rate.
...
\subsubsection{\textcolor{blue}{Paired statistical significance analysis}}
...
\begingroup\color{blue}
Figures~\ref{fig:stat_score_dist}--\ref{fig:stat_violin} offer three complementary views of the bootstrap distribution data, each reinforcing the conclusions drawn from Table~\ref{tab:paired_stats}.

Figure~\ref{fig:stat_score_dist} shows the raw score distributions for Average Fusion and the Proposed Method across 2{,}000 bootstrap resampling runs. The Proposed Method's distribution sits consistently to the right, reflecting higher median performance, yet the two distributions share considerable overlap throughout their range---a direct visual counterpart to the wide $\Delta$AUROC confidence interval $[-0.0754,\,0.0822]$. The degree of overlap makes clear that the observed gain, while real, does not decisively separate the two models on a split of this size.

Figure~\ref{fig:stat_kde_mean} smooths the same data into kernel density estimates, with dashed vertical lines marking each model's mean. The Proposed Method's curve peaks slightly to the right and its mean exceeds that of Average Fusion, consistent with the positive point estimates in Table~\ref{tab:paired_stats}. What is equally apparent, however, is the width of the shared density region: across a substantial share of resampled test sets the two models produce scores that are practically indistinguishable. Improvements of this magnitude are not unusual for incremental architectural advances on small datasets, where conventional significance thresholds are rarely cleared.

Figure~\ref{fig:stat_violin} displays the interquartile range and full score spread for both models. The Proposed Method's violin sits at a higher median and fans outward toward the upper tail, capturing the subset of resampled configurations where attention-based fusion pulls noticeably ahead. The lower tails of both violins overlap substantially, which aligns with the non-significant McNemar statistic ($p = 0.885$)---neither model consistently dominates across all possible draws from the test set. Read alongside the other panels, the violin plot reinforces that the performance advantage of the Proposed Method is directionally reliable but not yet decisive at the available sample size ($n_{\mathrm{test}} \approx 126$).
\endgroup

\begin{figure}[!ht]
    \begin{center}
    \includegraphics[width=0.75\linewidth]{1.jpeg}
    \end{center}
    \caption{\textcolor{blue}{Score distributions of Average Fusion and the Proposed Method across bootstrap runs, showing a rightward shift for the Proposed Method with substantial overlap.}}
    \label{fig:stat_score_dist}
\end{figure}

\begin{figure}[!ht]
    \begin{center}
    \includegraphics[width=0.75\linewidth]{2.jpeg}
    \end{center}
    \caption{\textcolor{blue}{KDE curves with mean markers for both models. The Proposed Method's mean is higher, yet the overlapping densities indicate an incremental rather than dramatic gain.}}
    \label{fig:stat_kde_mean}
\end{figure}

\begin{figure}[!ht]
    \begin{center}
    \includegraphics[width=0.75\linewidth]{3.jpeg}
    \end{center}
    \caption{\textcolor{blue}{Violin plots showing score spread and median for each model. The Proposed Method achieves a higher median and wider upper tail, though lower-tail overlap tempers strong conclusions.}}
    \label{fig:stat_violin}
\end{figure}

\subsection{Comparison with State-of-the-Art Methods}
Table~\ref{tab:sota} situates our results alongside published methods for mango and general fruit disease detection. Because these methods were evaluated on different datasets under different protocols, numerical comparisons must be interpreted carefully; the table is meant to provide context rather than a controlled benchmark. Our method is notable in that it achieves 87.3\% without any specialized hardware---a meaningful result given that comparable work typically relies either on controlled-condition datasets or more powerful sensors.
...
\begin{itemize}
    \item \textcolor{blue}{Co-registering RGB and thermal mango-fruit imagery if public or consortial paired datasets become available, to benchmark synthetic maps against measured radiance}
...
\textcolor{blue}{This represents a 4.8\% improvement over the strongest RGB-only baseline (ViT-Base at 85.8\%) and reaches approximately 89\% of the performance obtained using a real thermal imaging system (93.5\%), while requiring no additional hardware. The approach is well-suited for deployment in smartphone-based applications, enabling practical multi-modal disease diagnosis in field conditions without the cost and complexity of specialized imaging equipment.}
