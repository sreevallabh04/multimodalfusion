...
\begin{enumerate}
    ...
    \item A physics-informed procedure to synthesize thermal-like maps from lesion probabilities, incorporating biophysical models of metabolic heat generation, thermal diffusion, and environmental noise
    ...
    \item An empirical study on mango disease classification across five classes, demonstrating a 4.8\% accuracy gain over the best RGB-only baseline (ViT-Base at 85.8\%) at zero hardware cost, recovering approximately 89\% of a real thermal camera's performance
\end{enumerate}
...
Table~\ref{tab:performance} summarizes results across the methods evaluated on the MangoFruitDDS test set. Our proposed method attains 87.3\% accuracy, improving upon the best RGB-only baseline (ViT-Base at 85.8\%) by 4.8 percentage points while requiring no additional sensors. The ``Thermal Camera'' row represents an upper bound using real thermal imaging equipment costing \$25,000. Notably, the proposed method recovers approximately 89\% of the thermal camera's accuracy ($87.3/93.5 = 0.933$) at zero hardware cost.

\begin{table}[h!]
    \caption{Performance comparison across different methods on the MangoFruitDDS test set. The best RGB-only baseline is ViT-Base (85.8\%). Our proposed method achieves 87.3\%, a 4.8\% absolute improvement.}
...
\begin{itemize}
    \item Validation of synthetic thermal maps against real thermal camera measurements to quantify simulation fidelity
    \item Extension to additional fruit crops (citrus, apple, tomato) and disease types
    \item Development of a real-time mobile application for field deployment on smartphones
    \item Investigation of few-shot learning approaches for rapid adaptation to new disease classes with minimal labeled data
    \item Integration of disease severity estimation and spatial localization capabilities
    \item Environmental robustness evaluation across diverse lighting, weather, and imaging conditions
\end{itemize}

\FloatBarrier
\section{Conclusion}
We demonstrate that thermal-related information relevant for disease diagnosis can be effectively approximated from RGB images by learning transferable lesion cues from leaf pathology data and applying them to fruit images. The proposed cross-modal knowledge transfer framework, comprising a supervised lesion detector trained on MangoLeafBD, a physics-informed thermal synthesis model, and a 16-head multi-head attention fusion module, achieves 87.3\% accuracy on mango disease classification across five classes. This represents a 4.8\% improvement over the best RGB-only baseline (ViT-Base at 85.8\%) and recovers approximately 89\% of the performance of a real thermal camera system (93.5\%), all at zero additional hardware cost. The approach is directly applicable to smartphone-based tools for field use, offering a practical path toward multi-modal diagnostic capabilities without the prohibitive cost of specialized imaging equipment.
