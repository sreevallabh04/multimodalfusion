...
def monitor_log_files() -> Path | None:
    """
    Monitor training log files for progress.
    Returns:
        Path | None: Path to the latest log file or None if not found.
    """
    log_dir = Path("logs")
    logger = logging.getLogger("MonitorTraining")
    logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
    if not log_dir.exists():
        logger.info("\U0001F4C1 No logs directory found yet...")
        return None
    log_files = list(log_dir.glob("*.log"))
    if not log_files:
        logger.info("\U0001F4C4 No log files found yet...")
        return None
    latest_log = max(log_files, key=lambda x: x.stat().st_mtime)
    return latest_log

def parse_training_progress(log_file: Path) -> dict | None:
    """
    Parse training progress from log file.
    Args:
        log_file (Path): Path to the log file.
    Returns:
        dict | None: Progress dictionary or None if not found.
    """
    logger = logging.getLogger("MonitorTraining")
