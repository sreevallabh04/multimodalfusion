...
\noindent We analyze the confusion matrices for different methods to understand the class-wise performance.
These confusion matrices indicate that the proposed method reduces confusions for visually similar classes (e.g., Stem/Rot vs. Anthracnose) compared to the RGB baseline.
The stronger concentration along the diagonal suggests improved class separability due to the synthetic thermal cues and attention-based fusion.
...
\noindent We further evaluate the model performance using Receiver Operating Characteristic (ROC) curves.
Across all disease classes, the proposed method’s ROC curves stay closer to the top-left corner, indicating higher true-positive rates for comparable false-positive rates.
This confirms that fusing RGB with the synthetic thermal representation improves robustness across decision thresholds for practical field deployment.
